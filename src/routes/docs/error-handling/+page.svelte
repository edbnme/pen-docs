...
Start Chrome with: chrome --remote-debugging-port=9222`}
  ...
  <ul>
    ...
    <li><code>pen_list_pages</code> still works (it queries the browser, not a specific tab)</li>
    <li>The LLM should call <code>pen_list_pages</code> → <code>pen_select_page</code> to pick a live target</li>
  </ul>
  ...
  <p>Network response bodies can be big. Here's how PEN handles the edge cases:</p>

  <div class="table-wrapper">
    <table>
      <thead><tr><th>Scenario</th><th>Handling</th></tr></thead>
      <tbody>
        <tr><td>Response body &gt; limit</td><td>Truncated with <code>[truncated at N bytes]</code> suffix</td></tr>
        <tr><td>Binary response</td><td>Not captured (only text-based MIME types)</td></tr>
        <tr><td>Streaming response</td><td>Captured when complete</td></tr>
        <tr><td>No response (canceled)</td><td>Marked as <code>(canceled)</code> in waterfall</td></tr>
      </tbody>
    </table>
  </div>
  ...
  <ul>
    <li><strong>Missing source map</strong>: PEN serves the minified source as-is</li>
    <li><strong>Bad source map URL</strong>: Logged, skipped, minified source returned</li>
    <li><strong>Cross-origin source map</strong>: Can't fetch via CDP; minified source returned</li>
  </ul>
  ...
  <p>MCP allows concurrent tool calls. PEN handles this with <code>OperationLock</code>:</p>
  ...
  <ol>
    ...
    <li>The second caller gets an immediate error: <em>"HeapProfiler is already in use by another operation. Wait for the current heap snapshot to finish, or call another tool in the meantime."</em></li>
    <li>The error is returned as a <code>CallToolResult</code>, not an MCP protocol error</li>
  </ol>
  ...
  <div class="table-wrapper">
    <table>
      ...
      <tbody>
        <tr><td><code>HeapProfiler</code></td><td><code>pen_heap_snapshot</code></td></tr>
        <tr><td><code>HeapProfiler.tracking</code></td><td><code>pen_heap_track</code></td></tr>
        <tr><td><code>Profiler</code></td><td><code>pen_cpu_profile</code>, <code>pen_js_coverage</code></td></tr>
        <tr><td><code>Tracing</code></td><td><code>pen_capture_trace</code></td></tr>
        <tr><td><code>CSS</code></td><td><code>pen_css_coverage</code></td></tr>
        <tr><td><code>Lighthouse</code></td><td><code>pen_lighthouse</code></td></tr>
      </tbody>
    </table>
  </div>
  ...
  <p>
    ...
    error with the remaining wait time (e.g., <em>"pen_heap_snapshot has a 10s
    cooldown. Try again in 6s"</em>). Everything else has no cooldown.
  </p>

  <p>Rate limits exist to stop runaway LLM loops from hammering expensive operations.</p>
  ...
  <div class="table-wrapper">
    <table>
      ...
      <tbody>
        <tr><td>URL scheme</td><td><code>pen_navigate</code></td><td>Only <code>http://</code>, <code>https://</code> allowed</td></tr>
        <tr><td>Path traversal</td><td><code>pen_source_content</code></td><td>No <code>../</code> sequences</td></tr>
        <tr><td>Integer bounds</td><td>Various</td><td><code>topN</code> must be &gt; 0</td></tr>
        <tr><td>String length</td><td>Various</td><td>URLs capped at 2048 chars</td></tr>
        <tr><td>Required fields</td><td>All tools</td><td>Schema-enforced by MCP SDK</td></tr>
      </tbody>
    </table>
  </div>

  <p>Validation functions live in <code>internal/security/validate.go</code>.</p>
  ...
  <ul>
    ...
    <li><strong>Snapshot from a different page</strong>: Works, but the diff probably won't mean much</li>
  </ul>
  ...
  <p>
    <code>pen_capture_trace</code> takes a <code>duration</code> param (default:
    5s). Keep it short for sane file sizes.
  </p>
  ...
  <p>
    ...
    <code>defer cleanupTempDir(logger)</code> in <code>main.go</code>. Individual
    handlers also clean up via <code>defer os.Remove(f.Name())</code> after
    processing.
  </p>
  ...
  <ol>
    <li><strong>Never crash</strong>: PEN stays up. Errors go back as tool results.</li>
    <li><strong>Be specific</strong>: Say what happened and what to do about it.</li>
    <li><strong>Degrade gracefully</strong>: Partial results beat no results (truncated payloads, missing source maps, etc.).</li>
    <li><strong>Clean up after yourself</strong>: Every operation resets its own state, even on failure (<code>defer</code> in Go).</li>
    <li><strong>Let the LLM decide</strong>: PEN reports the problem. The LLM figures out what's next.</li>
  </ol>
</DocPage>
