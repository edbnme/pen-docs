...
>
  ...
  <div class="table-wrapper">
    <table>
      ...
      <tbody>
        <tr
          ><td>MCP ← LLM</td><td>Malicious eval, read sensitive files</td><td
            >Eval gating, expression blocklist, path validation</td
          ></tr
        >
        <tr
          ><td>MCP ← LLM</td><td>Navigate to dangerous URLs</td><td
            >URL scheme validation</td
          ></tr
        >
        <tr
          ><td>PEN → Chrome</td><td>Full browser access</td><td
            >Localhost-only CDP</td
          ></tr
        >
        <tr
          ><td>PEN → File System</td><td>Temp files, path traversal</td><td
            >Temp dir isolation, path checks</td
          ></tr
        >
        <tr
          ><td>PEN → Network</td><td>Source map fetching</td><td
            >Not implemented (by design)</td
          ></tr
        >
      </tbody>
    </table>
  ...
  <p>
    <code>pen_evaluate</code> is the scariest tool — it runs JS straight in the browser.
  </p>
  ...
  <p>
    Even with <code>--allow-eval</code> on, PEN blocks known-dangerous patterns:
  </p>
  ...
  <div class="table-wrapper">
    <table>
      ...
      <tbody>
        ...
        <tr
          ><td><code>XMLHttpRequest</code></td><td>Network exfiltration</td></tr
        >
        <tr
          ><td><code>navigator.sendBeacon</code></td><td>Data beaconing</td></tr
        >
        <tr
          ><td><code>localStorage</code></td><td>Persistent storage access</td
          ></tr
        >
        <tr
          ><td><code>sessionStorage</code></td><td>Session storage access</td
          ></tr
        >
  ...
  <p>
    Unicode escape sequences (<code>\uXXXX</code>) are also rejected to prevent
    sneaking past the blocklist.
  </p>
  ...
  <h2 id="gate-3-path-traversal-prevention">
    Gate 3: Path Traversal Prevention
  </h2>

  <p>
    Source tools accept file paths. <code>ValidateSourcePath</code> stops traversal
    cold:
  </p>
  ...
  <h2 id="gate-4-cdp-localhost-restriction">
...
pen --cdp-url http://localhost:9222`}
  ...
  <p>
    <code>pen_navigate</code> and <code>pen_lighthouse</code> validate URLs before
    any action:
  </p>
  ...
  <div class="table-wrapper">
    <table>
      ...
      <tbody>
        ...
        <tr
          ><td><code>chrome:</code> / <code>about:</code></td><td
            >Internal browser pages</td
          ></tr
        >
        <tr><td><code>ftp:</code></td><td>Unencrypted protocol</td></tr>
        <tr
          ><td><code>ws:</code> / <code>wss:</code></td><td
            >WebSocket connections</td
          ></tr
        >
  ...
  <ul>
    <li>
      Directory: <code>os.TempDir()/pen/</code> with <code>0700</code> permissions
    </li>
    <li>Files: <code>0600</code> permissions (owner-only read/write)</li>
    <li>
      Cleaned up on normal exit and on context cancellation via <code
        >defer</code
      >
    </li>
  </ul>
  ...
  <ol>
    <li>
      <strong>Default:</strong> Tool doesn't exist. MCP says "unknown tool."
    </li>
    <li>
      <strong>With <code>--allow-eval</code>:</strong> Blocked by the expression
      filter (<code>fetch(</code> pattern).
    </li>
    <li>
      <strong>Obfuscated:</strong> Regex might miss it — which is exactly why Gate
      1 (not enabling eval) is your real line of defense.
    </li>
  </ol>

  <h3>Path traversal via source tools</h3>

  <CodeBlock lang="json" code={`{ "urlPattern": "../../../../etc/passwd" }`} />
...
pen_heap_snapshot → blocked (cooldown: 10s)`}
  ...
  <p>
    Blocked immediately. <code>javascript:</code> isn't allowed. The request never
    reaches Chrome.
  </p>
  ...
  <div class="table-wrapper">
    <table>
      ...
      <tbody>
        <tr
          ><td><code>pen_evaluate</code> requires <code>--allow-eval</code></td
          ><td>&#10003;</td></tr
        >
        <tr
          ><td>Expression blocklist + Unicode escape check</td><td>&#10003;</td
          ></tr
        >
        <tr
          ><td>Path traversal prevention (source paths)</td><td>&#10003;</td
          ></tr
        >
        ...
        <tr
          ><td>URL scheme validation (navigate/lighthouse)</td><td>&#10003;</td
          ></tr
        >
  ...
  <ul>
    <li>
      <strong>Never enable <code>--allow-eval</code> in production</strong> or untrusted
      environments
    </li>
    <li>
      <strong>Always set <code>--project-root</code></strong> to limit source tool
      access
    </li>
    <li><strong>Never expose port 9222</strong> to the network</li>
    <li><strong>Use SSH tunneling</strong> for remote browsers</li>
    <li>
      <strong>Use a reverse proxy with auth</strong> if exposing PEN's HTTP transport
    </li>
  </ul>
</DocPage>
